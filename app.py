...
def save_selected_text():
    ...
    try:
        ...
        # Save the selected text and response message to the database
        conn = get_db_connection()
        cur = conn.cursor()

        # Insert data into the database
        insert_query = sql.SQL("""
            INSERT INTO userDetail (selected_text, response_message)
            VALUES (%s, %s)
        """)
        cur.execute(insert_query, (selected_text, response_message))
        conn.commit()
        ...
        return jsonify({"message": "Selected text processed and saved successfully", "response": response_message})
